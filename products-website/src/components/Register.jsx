...
const Register = () => {
  ...
  const allReqsMet = reqLength && reqUpper && reqLower && reqNumber;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('https://zenith-e-commerce-store.vercel.app/api/users/register', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="max-w-md mx-auto mt-16 p-8 bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl">
      ...
      {error && <p className="text-red-400 mb-6 text-center font-medium bg-red-400/10 p-3 rounded-lg border border-red-400/20">{error}</p>}
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              required
              className="w-full pl-4 pr-12 py-3 rounded-xl bg-gray-950 border border-gray-800 text-white placeholder-gray-600 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all"
              placeholder="••••••••"
            />
            <button
              type="button"
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
            <span className={reqLength ? "text-emerald-400" : "text-gray-500"}>
              {reqLength ? "✓" : "○"} At least 8 characters
            </span>
            <span className={reqUpper ? "text-emerald-400" : "text-gray-500"}>
              {reqUpper ? "✓" : "○"} One uppercase letter
            </span>
            <span className={reqLower ? "text-emerald-400" : "text-gray-500"}>
              {reqLower ? "✓" : "○"} One lowercase letter
            </span>
            <span className={reqNumber ? "text-emerald-400" : "text-gray-500"}>
        ...
        <button
          type="submit"
          className="w-full bg-indigo-600 text-white font-bold py-3 px-4 rounded-xl shadow-lg hover:shadow-indigo-500/30 hover:bg-indigo-500 transition-all duration-300 mt-2"
        >
          Register
        </button>
      </form>
